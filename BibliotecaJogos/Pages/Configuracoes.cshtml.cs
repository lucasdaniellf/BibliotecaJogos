...
namespace BibliotecaJogos.Pages
{
    public class ConfiguracoesModel : PageModel
    {
        ...
        public ConfiguracoesModel(IOptions<DataBaseContext> options, BibliotecaServiceNG bibliotecaNG, BibliotecaServiceAD bibliotecaAD )
        {
        ...
        public ActionResult OnPostDeleteEstudio(int id)
        {
            try
            {
                ...
                if (rows > 0)
                {
                    TempData["sucesso_config"] = "Estudio deletado com sucesso !";
                }
                return RedirectToPage("/Configuracoes");
            }
            catch (NpgsqlException)
        ...
        public ActionResult OnPostDeleteGenero(int id)
        {
            try
            {
                ...
                if (rows > 0)
                {
                    TempData["sucesso_config"] = "Genero deletado com sucesso !";
                }
                return RedirectToPage("/Configuracoes");
            }
            catch (NpgsqlException)
